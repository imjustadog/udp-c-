...
namespace FlyTcpFramework
{
    ...
    public class TcpSvr
    {
        ...
        public TcpSvr(IPAddress serverIP,ushort port, ushort maxClient, Coder coder)
        {
            _serverIP = serverIP;
            _port = port;
        ...
        /// <param name="port">服务器端监听的端口号</param> 
        /// <param name="maxClient">服务器能容纳客户端的最大能力</param> 
        public TcpSvr(IPAddress serverIP, ushort port, ushort maxClient)
        {
            _serverIP = serverIP;
            _port = port;
            _maxClient = maxClient;
        ...
        /// </summary> 
        /// <param name="port">服务器端监听的端口号</param> 
        public TcpSvr(IPAddress serverIP, ushort port)
        ...
        public virtual void SendText(Session recvDataClient, byte[] data)
        {
            //获得数据编码 
        ...
        protected virtual void ReceiveData(IAsyncResult iar)
        {
            ...
            try
            {
                ...
                //byte[] receivedData;
                //receivedData = _recvDataBuffer;

                //发布收到数据的事件 
                if (RecvData != null)
                {

                   Session clientSession = FindSession(client);
